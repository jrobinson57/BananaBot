...
async function rollD20andGiveRole(userId, currentStage) 
{
    ...
    // But this user DOES have the power banana (obviously)
    ///////////////////////////////////////////////////////////////
    if(!userProfile.iHaveBanana && userProfile.iHavePowerBanana) {
        ...
        } else if (userRoll >= 15 && userRoll < 21) {
    ...
    // But this user DOES have the power banana (obviously)
    ///////////////////////////////////////////////////////////////
    else if(userProfile.iHaveBanana && userProfile.iHavePowerBanana) {
        // Determine the next evolution stage based on the roll result
        if (userRoll === 1) {
            evolutionStage = Math.max(1, currentStage - 1); // Decrement by 1, ensuring it doesn't go below 1
        } else if (userRoll >= 14 && userRoll < 21) {
            // Progress to the next stage based on the stage map
            evolutionStage = currentStage;
            while (true) {
                evolutionStage++;
        ...
        console.log("User does not have the power banana");
        return;
    }
...
module.exports = {
    run: async ({ interaction }) => {
        ...
        try {
            ...
            if(!userProfile.iHavePowerBanana)
                {
                    console.log(interaction.user.username + " DOES NOT HAVE POWER BANANA!!!!");
                    await interaction.editReply({
                        content: `You do not have the power banana.`,
                ...
                if (evorollCoins < 1) {
                    ...
                    switch(result.userRoll) {
                        case 20:
                            await interaction.editReply(
                                `You rolled a ${result.userRoll} with the Power Banana. You evolved 2 times to evolution stage ${stageAnimalMap[result.evolutionStage]}. \nYou have ${userProfile.evorollCoins} evoroll coin(s) available.`
                            );
                          break;
                        case 19:
                            await interaction.editReply(
            ...
            console.log(`Error handling /powerbanana: ${error}`);
    ...
    data: {
        name: 'powerbanana',
        description: '15-20 rank you up twice, the rest is the same',
    },
};
