...
async function rollD20andGiveRole(userId, currentStage) 
{
    ...
    */
    ...
    // But this user DOES have the reality banana (obviously)
    ///////////////////////////////////////////////////////////////
    if(!userProfile.iHaveBanana && userProfile.iHaveRealityBanana) {
    ...
    // But this user DOES have the Reality banana (obviously)
    ///////////////////////////////////////////////////////////////
    else if(userProfile.iHaveBanana && userProfile.iHaveRealityBanana) {
        // Determine the next evolution stage based on the roll result
        if (userRoll === 1) {
            evolutionStage = currentStage;
            while (true) {
        ...
        console.log("User does not have the reality banana");
        return;
    }
...
module.exports = {
    run: async ({ interaction }) => {
        ...
        try {
            ...
            if(!userProfile.iHaveRealityBanana)
                {
                    console.log(interaction.user.username + " DOES NOT HAVE REALITY BANANA!!!!");
                    await interaction.editReply({
                        content: `You do not have the reality banana.`,
                ...
                if (evorollCoins < 1) {
                    ...
                    switch(result.userRoll) {
                        case 1:
                            await interaction.editReply(
                                `You rolled a ${result.userRoll} with the Reality Banana. You evolved one time to evolution stage ${stageAnimalMap[result.evolutionStage]}.`
                            );
                          break;
                        default:
                            await interaction.editReply(
            ...
            console.log(`Error handling /realitybanana: ${error}`);
    ...
    data: {
        name: 'realitybanana',
        description: '1s rank you up once, the rest is the same',
    },
};
